...
export function Medicines() {
  const { med, setMedQuery, selectMed, clearMed } = useStore()
  ...
  function buildSelected(m: typeof MEDICINES[number]) {
    ...
    return {
      ...
      farmacias: sorted.map((f) => {
        ...
        return { ...f, precioStr: f.precio.toFixed(2), isCheapest: cheap, borderColor: cheap ? '#0d7d74' : '#eaeeed', stockBg: c.bg, stockFg: c.fg, ahorroVs: (f.precio - min).toFixed(2), hasAhorro: f.precio > min }
      }),
    }
  }
  ...
  return (
    <div style={s('animation:hq-fade .35s ease both;')}>
      ...
      {!medSelected && (
        <div>
          <div style={s('font-size:12.5px;font-weight:700;letter-spacing:.04em;color:#6b7b79;margin-bottom:14px;')}>{medResults.length} MEDICAMENTOS ENCONTRADOS</div>
          ...
          {medResults.length === 0 && (
            <div style={s('background:#fff;border:1px dashed #d4e0de;border-radius:16px;padding:40px;text-align:center;color:#8a9a98;font-size:14px;')}>No se encontraron medicamentos para “{med.query}”. Prueba con Salbutamol, Paracetamol o Ibuprofeno.</div>
          )}
        </div>
      )}
      ...
      {medSelected && (
        <div style={s('animation:hq-fade .3s ease both;')}>
          ...
          <div style={s('font-size:12px;color:#a3b1af;margin-top:16px;text-align:center;')}>Precios referenciales del mercado · Integración prevista con farmacias (RENIEC/farmacias, evolución futura del DAS).</div>
        </div>
      )}
    </div>
  )
