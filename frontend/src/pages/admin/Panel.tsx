...
export function AdminPanel() {
  const { allUsers, tokens, audit, now, history } = useStore()
  ...
  return (
    <div data-testid="admin-panel" style={s('animation:hq-fade .35s ease both;')}>
      ...
      <div style={s('display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:18px;')}>
        <div style={s(STAT)}><div style={s('font-size:12.5px;color:#516160;margin-bottom:8px;')}>Usuarios registrados</div><div style={s('font-size:30px;font-weight:800;')}>{allUsers.length}</div></div>
