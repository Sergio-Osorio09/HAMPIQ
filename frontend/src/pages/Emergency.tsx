      ...
      <div style={s('flex:1;display:flex;align-items:center;justify-content:center;padding:40px;')}>
        ...
        {!emg.loaded && (
          <div style={s('text-align:center;max-width:460px;animation:hq-fade .4s ease both;')}>
            ...
            <button onClick={escanearQR} style={s('border:none;background:#e23b48;color:#fff;font-size:15.5px;font-weight:700;padding:15px 28px;border-radius:12px;cursor:pointer;box-shadow:0 12px 28px -10px #e23b48;margin-bottom:20px;width:100%;')}>
              {emg.scanning
            ...
            <div style={s('display:flex;gap:9px;')}>
              <Box as="input" value={emg.codeInput} onChange={(e) => setEmg('codeInput', e.target.value)} placeholder="EMG-XXXXXXXX"
              ...
              <button onClick={validarCodigoEmergencia} style={s('border:none;background:#3a1f23;color:#fff;font-weight:600;font-size:14px;padding:0 20px;border-radius:11px;cursor:pointer;')}>Ver</button>
            </div>
            {emg.error && (
        ...
        {emg.loaded && (
          <div style={s('max-width:620px;width:100%;animation:hq-pop .4s ease both;')}>
            <div style={s('background:#fff;color:#0f211f;border-radius:22px;overflow:hidden;box-shadow:0 40px 80px -30px rgba(0,0,0,.6);')}>
              <div style={s('background:#e23b48;color:#fff;padding:22px 28px;display:flex;align-items:center;justify-content:space-between;')}>
                ...
                <div style={s('text-align:right;')}><div style={s('font-size:11px;font-weight:700;opacity:.85;letter-spacing:.06em;')}>GRUPO SANGUÍNEO</div><div style={s('font-size:32px;font-weight:800;line-height:1;')}>O+</div></div>
              </div>
              <div style={s('padding:26px 28px;')}>
                <div style={s('display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:18px;')}>
                  <div style={s('background:#fff4f5;border:1px solid #f8d4d8;border-radius:14px;padding:16px;')}><div style={s('font-size:12px;font-weight:700;color:#c0202f;letter-spacing:.04em;margin-bottom:8px;')}>⚠️ ALERGIAS</div><div style={s('font-size:15.5px;font-weight:700;')}>{alergiasStr}</div></div>
